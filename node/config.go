...
import (
	...
	"github.com/ChainAAS/gendchain/accounts/external"
	"github.com/ChainAAS/gendchain/accounts/keystore"
	"github.com/ChainAAS/gendchain/accounts/scwallet"
	"github.com/ChainAAS/gendchain/accounts/usbwallet"
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "GendChain", keep that.
...
// These resources are resolved differently for "gendchain" instances.
var isOldGendChainResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	if c.name() == "geth" && isOldGendChainResource[path] {
		oldpath := ""
		if c.Name == "geth" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "gendchain-keystore")
		ephemeral = keydir
	}
