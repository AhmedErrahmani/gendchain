...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", GendChain.LightScryptN, GendChain.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.gendchain", new NodeConfig());
			node.start();
...
func TestAndroid(t *testing.T) {
	...
	// Generate the mobile bindings for GendChain and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "io.gendchain", "github.com/ChainAAS/gendchain/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "io", "gendchain", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
