...
// DefaultGenesisBlock returns the GendChain mainnet genesis block.
//TODO: update mainnet genesis block params
...
// DefaultTestnetGenesisBlock returns the GendChain testnet network genesis block.
//TODO: update mainnet genesis block params
...
// DeveloperGenesisBlock returns the 'gendchain --dev' genesis block. Note, this must
...
// LocalGenesisBlock returns the 'gendchain --local' genesis block.
func LocalGenesisBlock(period uint64, signer common.Address, alloc GenesisAlloc) *Genesis {
	...
	// Assemble and return the genesis with the precompiled and seeds pre-funded
	return &Genesis{
		Config:     &config,
		Timestamp:  uint64(time.Now().Unix()),
		ExtraData:  append(extra, make([]byte, 32-len(extra))...),
