...
func (hr *Hasher) Size() int {
	return hr.size
}

// BlockSize returns the block size
func (hr *Hasher) BlockSize() int {
	return hr.blocksize
}
...
func (hr *Hasher) Sum(b []byte) (r []byte) {
	t := hr.bmt
	i := hr.cur
	n := t.leaves[i]
	j := i
	...
	if len(hr.segment) > hr.size && i > 0 && n.parent != nil {
		n = n.parent
	} else {
		i *= 2
	}
	d := hr.finalise(n, i)
	hr.writeSegment(j, hr.segment, d)
	c := <-hr.result
	hr.releaseTree()

	// sha3(length + BMT(pure_chunk))
	if hr.blockLength == nil {
		return c
	}
	res := hr.pool.hasher()
	res.Reset()
	_, _ = res.Write(hr.blockLength)
	_, _ = res.Write(c)
	return res.Sum(nil)
}
...
func (hr *Hasher) Write(b []byte) (int, error) {
	l := len(b)
	if l <= 0 {
		return 0, nil
	}
	...
	i := hr.cur
	count := (hr.count + 1) / 2
	need := hr.count*hr.size - hr.cur*2*hr.size
	size := hr.size
	if need > size {
		size *= 2
	}
...
func (hr *Hasher) ReadFrom(r io.Reader) (m int64, err error) {
	bufsize := hr.size*hr.count - hr.size*hr.cur - len(hr.segment)
...
func (hr *Hasher) ResetWithLength(l []byte) {
	hr.Reset()
	hr.blockLength = l
...
func (hr *Hasher) releaseTree() {
	if hr.bmt != nil {
		n := hr.bmt.leaves[hr.cur]
...
func (hr *Hasher) writeSegment(i int, s []byte, d int) {
	h := hr.pool.hasher()
	n := hr.bmt.leaves[i]

	if len(s) > hr.size && n.parent != nil {
		go func() {
			h.Reset()
			_, _ = h.Write(s)
			s = h.Sum(nil)
			...
			hr.run(n.parent, h, d, n.index, s)
		}()
		return
	}
	go hr.run(n, h, d, i*2, s)
}

func (hr *Hasher) run(n *Node, h hash.Hash, d int, i int, s []byte) {
	isLeft := i%2 == 0
	for {
		if isLeft {
			n.left = s
...
func (hr *Hasher) finalise(n *Node, i int) (d int) {
	isLeft := i%2 == 0
	for {
