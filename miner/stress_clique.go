...
func main() {
	...
	for _, node := range nodes {
		...
		if err := node.Service(&gendchain); err != nil {
			panic(err)
		}
		if err := gendchain.StartMining(1); err != nil {
			panic(err)
		}
	}
	time.Sleep(3 * time.Second)
	...
	for {
		...
		var gendchain *eth.GendChain
		if err := nodes[index%len(nodes)].Service(&gendchain); err != nil {
			panic(err)
		}
		...
		if err := gendchain.TxPool().AddLocal(tx); err != nil {
			panic(err)
		}
		nonces[index]++

		// Wait if we're too saturated
		if pend, _ := gendchain.TxPool().Stats(); pend > 2048 {
...
func makeGenesis(faucets []*ecdsa.PrivateKey, sealers []*ecdsa.PrivateKey) *core.Genesis {
	// Create a Clique network based off of the default config
...
func makeSealer(genesis *core.Genesis, nodes []string) (*node.Node, error) {
	// Define the basic configurations for the GendChain node
	...
	// Start the node and configure a full GendChain node on it
