// Package les implements the Light GendChain Subprotocol.
package les
...
func New(ctx context.Context, sctx *node.ServiceContext, config *eth.Config) (*LightGendChain, error) {
...
func (s *LightGendChain) APIs() []rpc.API {
...
func (s *LightGendChain) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *LightGendChain) BlockChain() *light.LightChain      { return s.blockchain }
func (s *LightGendChain) TxPool() *light.TxPool              { return s.txPool }
func (s *LightGendChain) Engine() consensus.Engine           { return s.engine }
func (s *LightGendChain) LesVersion() int                    { return int(s.protocolManager.SubProtocols[0].Version) }
func (s *LightGendChain) Downloader() *downloader.Downloader { return s.protocolManager.downloader }
func (s *LightGendChain) EventMux() *core.InterfaceFeed      { return s.eventMux }

// Protocols implements node.Service, returning all the currently configured
// network protocols to start.
func (s *LightGendChain) Protocols() []p2p.Protocol {
	return s.protocolManager.SubProtocols
}

// Start implements node.Service, starting all internal goroutines needed by the
// GendChain protocol implementation.
func (s *LightGendChain) Start(srvr *p2p.Server) error {
...
// GendChain protocol.
func (s *LightGendChain) Stop() error {
	s.odr.Stop()
