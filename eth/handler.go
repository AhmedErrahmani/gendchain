...
import (
	...
	"github.com/ChainAAS/gendchain/eth/fetcher"
...
// NewProtocolManager returns a new ethereum sub protocol manager. The GendChain sub protocol manages peers capable
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing GendChain peer", "peer", id)

	// Unregister the peer from the downloader and GendChain peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping GendChain protocol")
	...
	log.Info("GendChain protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("GendChain peer connected", "name", p.Name())

	// Execute the GendChain handshake
	var (
	...
	if err := p.Handshake(pm.networkId, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("GendChain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("GendChain peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Error("GendChain message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the GendChain sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // GendChain network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
