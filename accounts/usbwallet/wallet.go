...
type wallet struct {
	...
	deriveChain     gendchain.ChainStateReader // Blockchain state reader to discover used account with
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain gendchain.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
