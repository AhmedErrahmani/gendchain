...
import (
	...
	"github.com/ChainAAS/gendchain/goclient"
	"github.com/ChainAAS/gendchain/les"
	"github.com/ChainAAS/gendchain/log"
	"github.com/ChainAAS/gendchain/netstats"
	"github.com/ChainAAS/gendchain/node"
	"github.com/ChainAAS/gendchain/p2p"
	"github.com/ChainAAS/gendchain/p2p/discover"
	"github.com/ChainAAS/gendchain/p2p/discv5"
	"github.com/ChainAAS/gendchain/p2p/nat"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the GendChain protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for GendChain and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // GendChain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an GendChain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // GendChain protocol stack
	client *goclient.Client    // Client connection to the GendChain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the GendChain light client protocol
...
// close terminates the GendChain connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the GendChain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
...
// the username, avatar URL and GendChain address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for GendChain addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("no GendChain address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the GendChain address and profile URL.
...
// returning the username, avatar URL and GendChain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain GendChain address,
