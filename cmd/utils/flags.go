...
import (
	...
	"github.com/ChainAAS/gendchain/common/fdlimit"
	...
	"github.com/ChainAAS/gendchain/p2p/netutil"
	"github.com/ChainAAS/gendchain/params"
	whisper "github.com/ChainAAS/gendchain/whisper/whisperv6"
...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		Name: "networkid",
		//TODO: update nets ids
		Usage: "Network identifier (integer, 60=mainnet, 100=testnet)",
		Value: eth.DefaultConfig.NetworkId,
	}
	TestnetFlag = cli.BoolFlag{
		Name:  "testnet",
		Usage: "GendChain test network: pre-configured test network",
	}
...
// for GendChain and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gendchain account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an GendChain client to the stack.
func RegisterEthService(ctx context.Context, stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the GendChain service: %v", err)
	}
}
...
// RegisterNetStatsService configures the GendChain Stats daemon and adds it to
// the given node.
func RegisterNetStatsService(stack *node.Node, cfg netstats.Config) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both eth and les services
		var ethServ *eth.GendChain
		ctx.Service(&ethServ)

		var lesServ *les.LightGendChain
		ctx.Service(&lesServ)

		return netstats.New(cfg, ethServ, lesServ), nil
	}); err != nil {
		Fatalf("Failed to register the GendChain Stats service: %v", err)
	}
}
...
// new format.
//
// e.g. gendchain account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// gendchain --keystore /tmp/mykeystore --lightkdf account new
//
