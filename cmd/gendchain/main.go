// gendchain is the official command-line client for GendChain.
package main
...
import (
	...
	"github.com/ChainAAS/gendchain/console/prompt"
	...
	"github.com/ChainAAS/gendchain/internal/debug"
...
const (
	clientIdentifier = "GendChain" // Client identifier to advertise over the network
)
...
var (
	...
	app = utils.NewApp(gitCommit, "the GendChain command line interface")
...
func init() {
	// Initialize the CLI app and start GendChain
	app.Action = gendchain
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2022 The GendChain Authors"
...
// gendchain is the main entry point into the system if no special subcommand is run.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func gendchain(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	stack.AccountManager().Subscribe(events, "gendchain-startNode")
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) || ctx.GlobalBool(utils.LocalFlag.Name) {
		// Mining only makes sense if a full GendChain node is running
		if ctx.GlobalBool(utils.LightModeFlag.Name) || ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		var gendchain *eth.GendChain
		if err := stack.Service(&gendchain); err != nil {
			utils.Fatalf("GendChain service not running: %v", err)
		}
		...
		gendchain.TxPool().SetGasPrice(gasprice)

		threads := ctx.GlobalInt(utils.MinerLegacyThreadsFlag.Name)
		if ctx.GlobalIsSet(utils.MinerThreadsFlag.Name) {
			threads = ctx.GlobalInt(utils.MinerThreadsFlag.Name)
		}
		if err := gendchain.StartMining(threads); err != nil {
